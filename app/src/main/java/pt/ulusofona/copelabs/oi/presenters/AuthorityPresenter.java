    ...
    public void start(int authority) {
        ...
        switch (authority) {
            case NameModule.FIREFIGHTER_ID:
                title = mContext.getString(pt.ulusofona.copelabs.oi.R.string.firefighter);
                mPrefix = NameModule.getEmergencyPrefix(authority);
                break;
            case NameModule.POLICE_ID:
                title = mContext.getString(pt.ulusofona.copelabs.oi.R.string.police);
                mPrefix = NameModule.getEmergencyPrefix(authority);
                break;
            case NameModule.SPECIAL_SERVICE_ID:
                title = mContext.getString(pt.ulusofona.copelabs.oi.R.string.special_services);
                mPrefix = NameModule.getEmergencyPrefix(authority);
                break;
            case NameModule.RESCUE_TEAM_ID:
                title = mContext.getString(pt.ulusofona.copelabs.oi.R.string.rescue_team);
                mPrefix = NameModule.getEmergencyPrefix(authority);
                break;
        }
